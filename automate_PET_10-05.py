...
def main(weight, dose, folder):
    ...
    with open(folder + 'output.txt', 'w') as f:
        ...
        def bash_command_shell(*args):
            subprocess.run([str(c) for c in args], shell=True, universal_newlines=True, executable='/bin/bash')
        ...
        mask_SUV_patient = subprocess.check_output(['mincstats', '–mask', PET_subjectmask, '–mask_binvalue', mylist[-3], mylist_patient[-2], '–mean'], universal_newlines = True)
