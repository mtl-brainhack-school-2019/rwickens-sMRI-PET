...
def main(weight, dose, folder):
    ...
    with open(folder + 'output.txt', 'w') as f:
        ...
        folder = Path(folder)
        PET_subjectmask = splice(folder, '_subjectmask').with_suffix('.mnc')
        print("PET_subjectmask", PET_subjectmask)
        bash_command('mincresample', '-like', mylist_patient[-1],'-nearest', '-transform', outputPETpath_xfm, '-invert_transformation', mask_or_atlas_path, PET_subjectmask)
        # Getting major errors here
        ...
        mask_SUV_patient = subprocess.check_output(['mincstats', '-mask', PET_subjectmask, '–mask_binvalue', maskbinvalue, mylist_patient2, '–mean'], universal_newlines = True)       
        # write output to file
        # make multiple maskbinvalues possible here (csv); requires settung shell=True; come up with method
        ...
        for i in blurlist_patient: 
            ...
            templist.append(outputPETpath_patient,   + "_" + str(i) + "_blur.mnc")
            bash_command('mincblur', '-fwhm', i, mylist_patient[-2], outputPETpath_patient)
        for x in templist_patient:
            mylist_patient.append(templist[x])
