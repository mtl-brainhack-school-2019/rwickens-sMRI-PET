...
def splice(path: Path, modifier) -> Path:
    ...
    base_name = str(os.path.basename)
    if dir_name.count(".") > 1:
...
def main(weight, dose, folder):
    ...
    if patient_dir.exists() == False:  
        ...
        # In my case, patient folder path will rarely build on cwd - many ppl will use a shared folder on virtual machine that doesn't follow working directory format
        #I assume that this problem will go away with installer file? 
        raise SystemExit()
    ...
    for root, dirs, files in os.walk(patient_dir): #Set to max depth 3
    ...
    with open('output' + folder + '.txt', 'w') as f: # to-do: write to subject folder
