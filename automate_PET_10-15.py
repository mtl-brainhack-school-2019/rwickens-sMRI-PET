...
def main(weight, dose, folder):
    ...
    with open(folder + 'output.txt', 'w') as f:
        ...
        print("the mask radioactivity amount is", mask_SUV)         
        ...
        PETsubjectmask = str('PET_subjectmask.mnc')

        bash_command('mincresample', '-like', mylist_patient[-2], '-nearest', '-transform', outputPETpath_xfm, '-invert_transformation', mask_or_atlas_path, 'PET_subjectmask.mnc')

        patient_avg_suv = str(mylist_patient[-2])

        maskbinvalue = str(maskbinvalue)

        mask_SUV_patient = subprocess.check_output(['mincstats', '-mask', PETsubjectmask, '-mask_binvalue', maskbinvalue, patient_avg_suv, '-mean'], universal_newlines = True)

        print(mask_SUV_patient)

        mask_SUV_patient = str(mask_SUV_patient)

        print("mask_SUV_patient is", mask_SUV_patient)

        mask_SUV_patient = mask_SUV_patient.strip()

        mask_SUV_patient = re.sub("[^0-9.]", "", mask_SUV_patient)

        print("the patient mask radioactivity amount is", mask_SUV_patient)        

        outputPETpath_patient = splice(mylist_patient[-1], '_patient_SUVR')

        #change to mylist_patient -2 instead of -1?
